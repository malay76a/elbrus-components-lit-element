...
const NATIVE_PREFIX$1 = NATIVE_PREFIX; // Object on which raw native methods are stored.
  ...
  if (hasDescriptors) {
    ...
    defineNativeAccessors(Element.prototype, {
      ...
      innerHTML: {
        /** @this {Element} */
        get() {
          return getInnerHTML(this, n => n[NATIVE_PREFIX$1 + 'childNodes']);
        },
...
function removeEventListener(type, fnOrObj, optionsOrCapture) {
  ...
  let target = optionsOrCapture && optionsOrCapture.__shadyTarget || this; // Search the wrapped function.
...
const recordInsertBefore = (node, container, ref_node) => {
  ...
  if (node.nodeType === Node.DOCUMENT_FRAGMENT_NODE) {
    let c$ = node[SHADY_PREFIX + 'childNodes'];
...
const recordChildNodes = node => {
  const nodeData = ensureShadyDataForNode(node);

  if (nodeData.firstChild === undefined) {
    // remove caching of childNodes
    nodeData.childNodes = null;
    const first = nodeData.firstChild = node[NATIVE_PREFIX + 'firstChild'] || null;
...
function currentScopeIsCorrect(node, newScopeName) {
  ...
  if (node.nodeType === Node.DOCUMENT_FRAGMENT_NODE) {
    ...
    const childNodes = node[SHADY_PREFIX + 'childNodes'];
...
const NodePatches = getOwnPropertyDescriptors({
  ...
  get textContent() {
    if (isTrackingLogicalChildNodes(this)) {
      ...
      for (let i = 0, cn = this[SHADY_PREFIX + 'childNodes'], c; c = cn[i]; i++) {
  ...
  insertBefore(node, ref_node) {
    ...
    const needsScoping = (!preferPerformance || node['__noInsertionPoint'] === undefined) && !currentScopeIsCorrect(node, newScopeName);
  ...
  cloneNode(deep) {
    if (this.localName == 'template') {
      ...
      if (deep && n.nodeType !== Node.ATTRIBUTE_NODE) {
        let c$ = this[SHADY_PREFIX + 'childNodes'];
...
const ParentNodeDocumentOrFragmentPatches = settings.preferPerformance ? Object.assign({}, ParentNodePatches) : ParentNodePatches;
...
const ElementOrShadowRootPatches = getOwnPropertyDescriptors({
  ...
  get innerHTML() {
    if (isTrackingLogicalChildNodes(this)) {
      ...
      /** @type {HTMLTemplateElement} */
      this.content : this;
      return getInnerHTML(content, e => e[SHADY_PREFIX + 'childNodes']);
    } else {
...
class ShadyRoot {
  ...
  _renderSelf() {
    ...
    if (!settings['preferPerformance'] && !this._hasRendered) {
      const c$ = this.host[SHADY_PREFIX + 'childNodes'];
  ...
  _composeNode(node) {
    ...
    for (let i = 0; i < c$.length; i++) {
      let child = c$[i]; // Note: if we see a slot here, the nodes are guaranteed to need to be
  ...
  _updateChildNodes(container, children) {
    let composed = Array.prototype.slice.call(container[NATIVE_PREFIX + 'childNodes']);
  ...
  _sortSlots(slots) {
    ...
    return slots.sort((a, b) => {
      ...
      for (var i = 0; i < listA.length; i++) {
        ...
        if (nA !== nB) {
          let c$ = Array.from(nA[SHADY_PREFIX + 'parentNode'][SHADY_PREFIX + 'childNodes']);
...
if (window['customElements'] && settings.inUse && !settings['preferPerformance']) {
  ...
  const define = window['customElements']['define']; // NOTE: Instead of patching customElements.define,
...
function PatchElement (internals) {
  ...
  function patch_innerHTML(destination, baseDescriptor) {
    Object.defineProperty(destination, 'innerHTML', {
      ...
      /** @this {Element} */
      function (htmlString) {
        const isConnected$$1 = isConnected(this); // NOTE: In IE11, when using the native `innerHTML` setter, all nodes
...
class StyleProperties {
  ...
  whenHostOrRootRule(scope, rule, cssBuild$$1, callback) {
    if (!rule.propertyInfo) {
      this.decorateRule(rule);
    }
  ...
  hostAndRootPropertiesForScope(scope, rules, cssBuild$$1) {
    let hostProps = {},
        rootProps = {}; // note: active rules excludes non-matching @media rules

    forEachRule(rules, rule => {
      // if scope is StyleDefaults, use _element for matchesSelector
      this.whenHostOrRootRule(scope, rule, cssBuild$$1, info => {
...
class ScopingShim {
  ...
  prepareTemplateStyles(template, elementName, typeExtension) {
    ...
    let cssBuild$$1 = getCssBuild(template);
    const optimalBuild = isOptimalCssBuild(cssBuild$$1);
    let info = {
      is: elementName,
    ...
    if (!optimalBuild) {
      let hasMixins = !cssBuild$$1 && detectMixin(cssText);
    ...
    if (!ownPropertyNames.length || nativeCssVariables) {
      ...
      let style = this._generateStaticStyle(info, template['_styleAst'], root, placeholder, cssBuild$$1, optimalBuild ? cssText : '');
  ...
  prepareTemplateDom(template, elementName) {
    ...
    const cssBuild$$1 = getCssBuild(template);
  ...
  _generateStaticStyle(info, rules, shadowroot, placeholder, cssBuild$$1, cssText) {
  ...
  _prepareHost(host) {
    ...
    const cssBuild$$1 = getCssBuild(template);
    const styleInfo = new StyleInfo(ast, placeholder, ownStylePropertyNames, is, typeExtension, cssBuild$$1);
  ...
  transformCustomStyleForDocument(style) {
    const cssBuild$$1 = getCssBuild(style);
...
//# sourceMappingURL=chunk-020bc6bb.js.map
